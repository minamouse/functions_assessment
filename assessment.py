...
def full_name(first, last):
    """Concatenates first and last name into one string.
    
    For example:
    >>> full_name('Marina', 'Cottrell')
    'Marina Cottrell'
...
def thing(first, last, town):
    """Prints a message if we're from the same place or not.

    For example:
    >>> thing('David', 'Morgan', 'Chicago')
    Hi, David Morgan, where are you from?
...
def calculate_price(price, state, tax=0.05):
    ...
    full_price = price + (price * tax)

    if state == 'CA':
        full_price += price * 0.03
    elif state == 'PA':
        full_price += 2
    elif state == 'MA':
        if price >= 100:
...
def add_alot(lst, *args):
    """Appends items to a list

    >>> add_alot([1, 3, 6, 2], 4, 7, 1, 8)
    [1, 3, 6, 2, 4, 7, 1, 8]
...
def get_word(word):
    """Return word and word multiplied by 3.

    >>> get_word('yo')
    ('yo', 'yoyoyo')

    >>> get_word('Balloonicorn')
    ('Balloonicorn', 'BalloonicornBalloonicornBalloonicorn')
    """
